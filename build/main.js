...
function toggleNight(){
   ...
   togBtn.classList.toggle('tog-night-color')
   nav_arr.forEach(li=>{
...
function scrollTextToCenter(){
   ...
   directives.forEach((dir,index)=>{
      if(index%2==0){
         ...
         dir.style=`transform:translate(${(((window.scrollY*2)-body.clientHeight))}px,0);`
      }
      else{
         dir.classList.remove('wh-center-seq')
         dir.classList.add('wh-right-seq')
         dir.style=`transform:translate(${(((window.scrollY*2)-body.clientHeight)*-1)}px,0);`
